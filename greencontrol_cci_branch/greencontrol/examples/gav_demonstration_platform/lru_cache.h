...
/// default cache specifications (in words LRU_cache::word_size)
...
//#define LRU_DETAILED_VERBOSE
#ifdef LRU_DETAILED_VERBOSE
#  define LRU_DETAILED_TRACE(msg, ...) { printf("@%s /%d (%s): ", sc_core::sc_time_stamp().to_string().c_str(), (unsigned)sc_core::sc_delta_count(), name()); printf(msg, ##__VA_ARGS__); printf("\n"); }
#else
#  define LRU_DETAILED_TRACE(msg, ...)
#endif
 ...
 *
 * When reading/writing lines from/back to a memory, <cache_line_size>-bit-aligned 
 * addresses are used!  Make sure that the addresses exist!
 *
 * Analysis abilities:
 * This cache provides several hit rate analysis parameters:
 * - Overall hit rate
 * - Hit rate of a specific address range
 *   The address range can be specified using configurable parameters of this module:
 *   hitrate_upper_addr and hitrate_lower_addr
 *
 */
class LRU_cache
...
private:
  ...
  gs::av::GAV_Api gavApi;
  
  /// StatCalc representing the cache overall hit rate
  ...
  /// Upper address for address range hit rate
  gs::gs_param<gs_uint64> hitrate_upper_addr;
  /// Lower address for address range hit rate
  gs::gs_param<gs_uint64> hitrate_lower_addr;
  /// StatCalc representing the cache memory address range hit rate
  gs::av::StatCalc<double> *addr_range_hit_rate;
  /// Overall Hits
  gs::gs_param<unsigned int> addr_range_hits;
  /// Overall Misses
  gs::gs_param<unsigned int> addr_range_misses;
...
public:
  ...
  LRU_cache(sc_core::sc_module_name name, tlm::simplebusMasterPort* memoryPort)
    : sc_core::sc_module(name),
      ...
      hitrate_upper_addr("hitrate_upper_addr"),
      hitrate_lower_addr("hitrate_lower_addr"),
      addr_range_hits  ("addr_range_hits"),
      addr_range_misses("addr_range_misses")
  {
    ...
    gs::gs_param_base& hits   = overall_hits;    // prepare parameters for convenience operators
    gs::gs_param_base& misses = overall_misses;  // prepare parameters for convenience operators
    hr_calc->calc("*", 100, (*hr_calc)(hits / (*hr_calc)(hits + misses))); // set formula: 100*(hits / (hits+misses))
    overall_hit_rate = new gs::av::StatCalc<double>("overall_hit_rate", hr_calc); // create Statistics Calculator
    cout << "overall_hit_rate parameter name: " << overall_hit_rate->get_result_param()->getName() << endl;

    // Address range hit rate Statistics Calculator
    gs::av::Calculator<double> *ar_hr_calc // Create Calculator
      = new gs::av::Calculator<double>("addr_range_hit_rate");
    ar_hr_calc->enable_sloppy(); // allow division by zero
    gs::gs_param_base& arhits   = addr_range_hits;    // prepare parameters for convenience operators
    gs::gs_param_base& armisses = addr_range_misses;  // prepare parameters for convenience operators
    ar_hr_calc->calc("*", 100, (*ar_hr_calc)(arhits / (*ar_hr_calc)(arhits + armisses))); // set formula: 100*(hits / (hits+misses))
    addr_range_hit_rate = new gs::av::StatCalc<double>("addr_range_hit_rate", ar_hr_calc); // create Statistics Calculator
    cout << "addr_range_hit_rate parameter name: " << addr_range_hit_rate->get_result_param()->getName() << endl;
  }
  ...
  ~LRU_cache() {
    ...
    delete addr_range_hit_rate->get_calculator();
    delete addr_range_hit_rate;
  }
