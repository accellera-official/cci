...
#ifndef __VCDWithTd_OutputPlugin_h__
#define __VCDWithTd_OutputPlugin_h__
...
namespace av {
  ...
  GAV_REGISTER_PLUGIN(16, VCD_TD_FILE_OUT, VCDWithTd_OutputPlugin)
  ...
  public:
     ...
     */
    VCDWithTd_OutputPlugin(const char* file_name, event_listener<OutputPlugin_base> *ev_listn)
    : OutputPlugin_base(file_name, ev_listn, VCD_TD_FILE_OUT, false),
      filename(file_name),
      started_once(false),
      current_time(sc_time(0,SC_NS)),
      output_time(static_cast<sc_dt::uint64>(-1), false),   // "infinite"
      current_delta(0)
    {
  ...
  protected:
     ...
     */
    void init() {
      GAV_DUMP_N(name(), "Init VCDWithTD Output Plugin: Open file '"<<filename.c_str()<<"' to write observed parameters to.");
  ...
  public:
    ...
    void config_callback(gs_param_base &par) {
      ...
      // only perform output action when not paused (or stopped)
        ...
        else if (sc_core::sc_time_stamp() > output_time) {
          write_time_slot();
        }
        ...
        sc_time time_to_store(sc_core::sc_time_stamp() + gs::gs_quantumkeeper::getTD());
        history[time_to_store][par.getName()] = par.getString();
        if (time_to_store < output_time) {
          output_time = time_to_store;
        }
      }
    }
  ...
  protected:
    ...
    /// Writes old cached time slots
    void write_time_slot() {
      while (sc_core::sc_time_stamp() > output_time) {
        // write time field
        fprintf(outp_file, "#%lld\n", output_time.value());
        // write fields
        for (unsigned i=0; i<p_vec.size(); i++)
        {
          const name_value_map& current_map = history.begin()->second;
          name_value_map::const_iterator iter_m = current_map.find(p_vec[i]);
          if( iter_m != current_map.end() ) {
        ...
        // erase the written values from history
        history.erase(history.begin());
        // next output_time
        if (history.empty()) {
          output_time = sc_time(static_cast<sc_dt::uint64>(-1), false);
        } else {
          output_time = history.begin()->first;
        }
      }
    ...
    /// param history
    typedef std::map<std::string, std::string> name_value_map;
    typedef std::map<sc_time, name_value_map> param_history;
