...
// is included by gs_param_base.h
...
template<typename gs_param_base_T,  template<class T> class gs_param_T>
...
//         initialize-mode.
template<typename gs_param_base_T, class GCnf_private_Api_TMPL, template<class T> class gs_param_T>
class GCnf_Api_t
: public sc_object,
  ...
  public gc_port_if
{
  ...
  friend class GCnf_private_Api_T<gs_param_base_T, gs_param_T>;
  
  /// Typedef to the API type
  typedef GCnf_Api_t<gs_param_base_T, GCnf_private_Api_TMPL, gs_param_T> GCnf_Api;
...
public:
  ...
  static cnf_api* getApiInstance(sc_core::sc_module *mod) 
  {
    if (!mod) {
      ...
      // TODO: Kann man testen, ob der caller ein module ist? Dann nämlich DEPRECATED WARNING weil nicht auf private API überprüft werden kann
      //       wenn test nicht möglich, SC_INFO ausgeben. (Für toplevel APIs ist der call mit NULL erlaubt.)
    }
    else {
...
protected:
  ...
  /// Deprecated event pointer for new parameter event. If NULL, getNewParamEvent not yet called. (But registeration may alredy be done if callbacks have been registered) 
...
public:
  ...
  template<typename gs_param_base_T, class GCnf_private_Api_TMPL, template<class T> class gs_param_T>
  bool GCnf_Api_t<gs_param_base_T, GCnf_private_Api_TMPL, gs_param_T>::initialize_mode = true;
  
  // typedef GCnf_Api_t<gs_param_base, GCnf_private_Api_T> GCnf_Api; // see private API
