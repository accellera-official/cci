...
class parameter_owner : public sc_module
{
	public	:
		...
		{	
			...
			std::cout << "\n\t[OWNER C_TOR] : Creating new integer type cci-parameter with default value 10" << std::endl;
		...
		void run_owner (void)
		{
			while(1)
			{
				...
				std::cout << "\t[OWNER] : Setting new value to the 'int' type param to '15'"<< std::endl;				
				*int_param = 15;
				
				wait(5.0, SC_NS);
				std::cout << "@ " << sc_time_stamp() << std::endl;

				std::cout << "\n\t[OWNER] : Creating new float type cci-parameter with default value : 12.345" << std::endl;
