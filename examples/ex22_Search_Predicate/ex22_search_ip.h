...
public:
    ...
    {
        return (handle.get_basic_type() == cci::CCI_NUMBER_PARAM);
    }

    static bool
    string_type_predicate(const cci::cci_param_untyped_handle handle)
    {
        return (handle.get_basic_type() == cci::CCI_STRING_PARAM);
    }
    ...
    {
        if(handle.get_basic_type() == cci::CCI_REAL_PARAM
           || handle.get_basic_type() == cci::CCI_NUMBER_PARAM) {
