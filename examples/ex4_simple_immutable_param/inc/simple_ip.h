...
SC_MODULE(simple_ip) {
 public:
  ...
  SC_CTOR(simple_ip)
    ...
    {
      SC_THREAD(execute);
      XREPORT("@Ctor: Default vale specified for "<< param_1.get_name()<< " is 1");
      XREPORT("@Ctor: Default vale specified for "<< param_2.get_name()<< " is 2");
	
      expect("Ctor: immutable param:", "param_1",(int)param_1, 100);
      expect("Ctor: immutable param:", "param_2",(int)param_2, 2);
    }

  //! @note This function can be removed after the CCI implementation is in place
  void expect(const char* phase, const char *key, const int val, const int exp){
    if(val != exp){
      XREPORT_PLAIN("Warning: "<< phase<< key<< " Expected to have value = "<<exp<<" but has "<<val<<". TBD?");
    }
    else {
  ...
  void execute() {
    ...
    expect("@Run: ", "param_1", (int)param_1, 100);
    expect("@Run: ", "param_2", (int)param_2, 2);
