...
SC_MODULE(simple_ip) {
 ...
 SC_CTOR(simple_ip)
   ...
   : struc_param("struc_param", 0)
    {
      SC_THREAD(execute);
      XREPORT("Ctor: Default value of "<< struc_param.get_name()<< " is "<< struc_param); 
    }

  /// Dtor
  ~simple_ip() {
    XREPORT_PLAIN("Dtor: Current value of "<< struc_param.get_name()<< " is "<< struc_param); 
  }
  ...
  void before_end_of_elaboration()  {
    ...
    struc_param = 1;
    if( 0 == struc_param )  {
        XREPORT_ERROR("Before_End_Elab: Value of struc_param remains unchanged "<< struc_param); 
    }
    else if( 1 == struc_param ) {
        XREPORT("Before_End_Elab: Value of struc_param changed to: "<< struc_param); 
    }
    else    {
        XREPORT_ERROR("Before_End_Elab: Invalid update to struc_param, value changed to: "<< struc_param); 
        assert( 0 );
    }
  }
  ...
  void execute() {
    ...
    if( 1 == struc_param )  {
        ...
        XREPORT_WARNING("execute: Value of struc_param changed to: "<< struc_param); 
    }
    else    {
        XREPORT_ERROR("execute: Invalid update to struc_param, value changed to: "<< struc_param); 
        assert( 0 );
    }
