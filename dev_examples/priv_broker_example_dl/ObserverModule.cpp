...
void ObserverModule::main_action() {
  ...
  for (iter = vec.begin() ; iter < vec.end(); iter++) {
    ...
    //ss_show << "Parameter value = " << mBroker->get_param(*iter)/*->get_value().get_string()*/ << std::endl;
  }
  ...
  if (p != NULL) {
    ...
    //p->set_value("10000");  //set_value only implemented for string values, else throws exception
    i_p = 10000;
    //p->json_deserialize("10000");
  }
  else
