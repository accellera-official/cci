...
void show_cci_value_vector(const cci::cci_value_list& vec) {
...
void ParameterOwnerModule::main_action() {
  ...
  */   
  ...
  cci::cci_value int_v(-10);
  DEMO_DUMP(name(), "Set and get parameter Owner.int_param - value = " << int_v.get_int());
  int_param.set_cci_value(int_v);
  int inte = int_param.get_cci_value().get_int();
  DEMO_DUMP(name(), "  got " << inte << endl);
  
  cci::cci_value uint_v(10);
  DEMO_DUMP(name(), "Set and get parameter Owner.uint_param - value = " << uint_v.get_int());
  uint_param.set_cci_value(uint_v);
  unsigned int uint = uint_param.get_cci_value().get_int64();
  DEMO_DUMP(name(), "  got " << uint << endl);

  cci::cci_value bool_v(true);
  DEMO_DUMP(name(), "Set and get parameter Owner.bool_param - value = " << bool_v.get_bool());
  bool_param.set_cci_value(bool_v);
  bool boole = bool_param.get_cci_value().get_bool();
  DEMO_DUMP(name(), "  got " << boole << endl);

  cci::cci_value dbl_v(10.123);
  DEMO_DUMP(name(), "Set and get parameter Owner.double_param - value = " << dbl_v.get_double());
  double_param.set_cci_value(dbl_v);
  double dbl = double_param.get_cci_value().get_double();
  DEMO_DUMP(name(), "  got " << dbl << endl);
  
  cci::cci_value flt_v(98.765);
  DEMO_DUMP(name(), "Set and get parameter Owner.float_param - value = " << flt_v.get_double());
  float_param.set_cci_value(flt_v);
  float flt = float_param.get_cci_value().get_double();
  DEMO_DUMP(name(), "  got " << flt << endl);
  
  cci::cci_value str_v("This is a string");
  DEMO_DUMP(name(), "Set and get parameter Owner.str_param - value = " << str_v.get_string());
  str_param.set_cci_value(str_v);
  std::string stri = str_param.get_cci_value().get_string();
  DEMO_DUMP(name(), "  got \"" << stri << "\"" << endl);
  
  cci::cci_value ll_v((sc_dt::int64)1000000000000000001ull);
  DEMO_DUMP(name(), "Set and get parameter Owner.ll_param - value = " << ll_v.get_int64());
  ll_param.set_cci_value(ll_v);
  long long ll = ll_param.get_cci_value().get_int64();
  DEMO_DUMP(name(), "  got " << ll << endl);
  
  cci::cci_value uc_v(70);
  DEMO_DUMP(name(), "Set and get parameter Owner.uchar_param - value = " << (unsigned char) uc_v.get_int());
  uchar_param.set_cci_value(uc_v);
  unsigned char uc = uchar_param.get_cci_value().get_int();
  DEMO_DUMP(name(), "  got " << uc << endl);
  
  cci::cci_value sc_v(71);
  DEMO_DUMP(name(), "Set and get parameter Owner.schar_param - value = " << (signed char) sc_v.get_int());
  schar_param.set_cci_value(sc_v);
  signed char sc = schar_param.get_cci_value().get_int();
  DEMO_DUMP(name(), "  got " << sc << endl);
  
  cci::cci_value c_v(72);
  DEMO_DUMP(name(), "Set and get parameter Owner.char_param - value = " << (char) c_v.get_int());
  char_param.set_cci_value(c_v);
  char c = char_param.get_cci_value().get_int();
  DEMO_DUMP(name(), "  got " << c << endl);
  cci::cci_value c_vs('A');
  DEMO_DUMP(name(), "Set and get parameter Owner.char_param - value = " << (char)c_vs.get_int());
  char_param.set_cci_value(c_vs);
  c = char_param.get_cci_value().get_int();
  DEMO_DUMP(name(), "  got " << c << endl);
  
  cci::cci_value us_v(244);
  DEMO_DUMP(name(), "Set and get parameter Owner.ushort_param - value = " << (unsigned short) us_v.get_int());
  ushort_param.set_cci_value(us_v);
  unsigned short us = ushort_param.get_cci_value().get_int();
  ...
  cci::cci_value_list lst_v_lst;
  lst_v_lst.push_back(cci::cci_value("value1"));
  lst_v_lst.push_back(cci::cci_value("value2"));
  lst_v_lst.push_back(cci::cci_value("value3"));
  cci::cci_value lst_v(lst_v_lst);
  DEMO_DUMP(name(), "Set and get parameter Owner.strvec_param - value = "); show_cci_value_vector(lst_v_lst);
  strvec_param.set_cci_value(lst_v);
  cci::cci_value v_get = strvec_param.get_cci_value();
  ...
  */
  ...
  cci::cci_value scintbv_v(-1000);
  DEMO_DUMP(name(), "Set and get parameter Owner.scintbase_param - value = " << scintbv_v);
  scintb_param.set_cci_value(scintbv_v);
  DEMO_DUMP(name(), "  got " << scintb_param.get_cci_value() << endl);
  cci::cci_value scintbv_vs("-111000");
  DEMO_DUMP(name(), "Set and get parameter Owner.scintbase_param - value = " << scintbv_vs);
  scintb_param.set_cci_value(scintbv_vs);
  DEMO_DUMP(name(), "  got " << scintb_param.get_cci_value() << endl);
  
  cci::cci_value scuintbv_v(1000);
  DEMO_DUMP(name(), "Set and get parameter Owner.scuintbase_param - value = " << scuintbv_v);
  scuintb_param.set_cci_value(scuintbv_v);
  DEMO_DUMP(name(), "  got " << scuintb_param.get_cci_value() << endl);
  cci::cci_value scuintbv_vs("111000");
  DEMO_DUMP(name(), "Set and get parameter Owner.scuintbase_param - value = " << scuintbv_vs);
  scuintb_param.set_cci_value(scuintbv_vs);
  DEMO_DUMP(name(), "  got " << scuintb_param.get_cci_value() << endl);
  
  cci::cci_value scsigned_v(-3000);
  DEMO_DUMP(name(), "Set and get parameter Owner.scsigned_param - value = " << scsigned_v);
  scsigned_param.set_cci_value(scsigned_v);
  DEMO_DUMP(name(), "  got " << scsigned_param.get_cci_value() << endl);
  cci::cci_value scsigned_vs("-2000");
  scsigned_param.set_cci_value(scsigned_vs);
  DEMO_DUMP(name(), "Set and get parameter Owner.scsigned_param - value = " << scsigned_vs);
  DEMO_DUMP(name(), "  got " << scsigned_param.get_cci_value() << endl);
  
  cci::cci_value scunsigned_v(3000);
  DEMO_DUMP(name(), "Set and get parameter Owner.scunsigned_param - value = " << scunsigned_v);
  scunsigned_param.set_cci_value(scunsigned_v);
  DEMO_DUMP(name(), "  got " << scunsigned_param.get_cci_value() << endl);
  cci::cci_value scunsigned_vs("2000");
  DEMO_DUMP(name(), "Set and get parameter Owner.scunsigned_param - value = " << scunsigned_vs.get_string());
  scunsigned_param.set_cci_value(scunsigned_vs);
  DEMO_DUMP(name(), "  got " << scunsigned_param.get_cci_value() << endl);
  
  // deprecated data type not implemented in prototype
  //cci::cci_value scbit_v(1);
  //DEMO_DUMP(name(), "Set and get parameter Owner.scbit_param - value = " << scbit_v.get_int());
  //scbit_param.set_cci_value(scbit_v);
  //sc_dt::sc_bit scbit(scbit_param.get_cci_value().get_int());
  //DEMO_DUMP(name(), "  got " << scbit << endl);

  cci::cci_value sclogic_v(1);
  DEMO_DUMP(name(), "Set and get parameter Owner.sclogic_param - value = " << sclogic_v.get_int());
  sclogic_param.set_cci_value(sclogic_v);
  DEMO_DUMP(name(), "  got " << sclogic_param.get_cci_value() << endl);
  sclogic_param = sc_dt::Log_X;
  std::cout << "get cci_value X: " << sclogic_param.get_cci_value() << std::endl;
  cci::cci_value sclogic_v1("Z");
  std::cout << "set cci_value  Z " << std::endl;
  sclogic_param.set_cci_value(sclogic_v1);
  std::cout << "get cci_value: " << sclogic_param.get_cci_value() << std::endl << std::endl;
  ...
  cci::cci_value sctime_v;
    sctime_v.set_list()
      .push_back(10.123)
      .push_back("SC_NS");
  // alternative style string ???
  //cci::cci_value sctime_v2("10.123 SC_PS");
  DEMO_DUMP(name(), "Set and get parameter Owner.sctime_param - value = " << sctime_v );
  sctime_param.set_cci_value(sctime_v);
  std::cout << "raw from cci_value (real, string): "<< sctime_param.get_cci_value() << std::endl;
  sc_core::sc_time sctime( sctime_param.get_cci_value().get<sc_core::sc_time>() );
  DEMO_DUMP(name(), "  got " << sctime.to_string() << endl);
    
  cci::cci_value scint_v(-9223372036854775807ll);
  DEMO_DUMP(name(), "Set and get parameter Owner.scint64_param - value = " << scint_v);
  scint64_param.set_cci_value(scint_v);
  DEMO_DUMP(name(), "  got " << scint64_param.get_cci_value() << endl);
  
  cci::cci_value scuint_v(18446744073709551615ull);
  DEMO_DUMP(name(), "Set and get parameter Owner.scuint64_param - value = " << scuint_v.get_uint64());
  scuint64_param.set_cci_value(scuint_v);
  DEMO_DUMP(name(), "  got " << scuint64_param.get_cci_value() << endl);
  
  cci::cci_value scbigint_v("-170141183460469200000000000000000000000");
  DEMO_DUMP(name(), "Set and get parameter Owner.scbigint128_param - value = " << scbigint_v.get_string());
  scbigint128_param.set_cci_value(scbigint_v);
  DEMO_DUMP(name(), "  got " << scbigint128_param.get_cci_value() << endl);
  
  cci::cci_value scbiguint_v("340282366920938463463374607431760000000");
  DEMO_DUMP(name(), "Set and get parameter Owner.scbiguint128_param - value = " << scbiguint_v.get_string());
  scbiguint128_param.set_cci_value(scbiguint_v);
  DEMO_DUMP(name(), "  got " << scbiguint128_param.get_cci_value() << endl);
  ...
  cci::cci_value valStr("Hello");
  try {
    uint_param.set_cci_value(valStr);
...
#endif
  try {
    some_param.set_cci_value(valStr);
  } catch(sc_core::sc_report const & e) {
    switch ( cci::cci_report_handler::get_param_failure(e) ) {
      case cci::CCI_VALUE_FAILURE:
        std::cout << std::endl << name() << ": Caught " << e.what() << std::endl;
